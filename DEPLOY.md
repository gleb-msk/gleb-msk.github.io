...
- **GitHub Pages:** в настройках репозитория включите Pages, source — ветка (например, `main`), папка — корень или `/docs` с `index.html` внутри.

Главная страница по умолчанию — `index.html` (это и есть премиум-версия). Страница `index-premium.html` дублирует её для старых ссылок.
